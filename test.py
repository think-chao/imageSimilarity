...
if __name__ == '__main__':
    source_img = r'C:\Users\king\Pictures\car\下载.jpg'
    root_path = r'C:\Users\king\Pictures\car'
    img_prefix = 'C:/Users/king/Pictures/car/'
    for img in os.listdir(root_path):
        img = img_prefix + img
        img1 = Image.open(source_img)
        img2 = Image.open(img)
        so = DHash()
        hash1 = so.calculate_hash(img1)
        hash2 = so.calculate_hash(img2)
        diff = so.hamming_distance(hash1, hash2)
        print('diff{}:{}~~~~~~~~~~~~~~~~~~~~~'.format(img, diff))
        if diff < 30:
            img2.show()
